...
def evaluate_equation(
    ...
    # --- DIMENSION 1D: Force 1D Function / Slice Visualization ---
    if dimension == "1D":
        if "EXPLICIT_3D" in eq_type:
            # 1D Slice along y = 0
            ...
            # Substitute y = 0
            slice_expr = sym_expr.subs(sym_y, 0)
            
            x_min, x_max = ranges.get(var_x, (-10.0, 10.0))
            n_pts = max(300, resolution * 2)
            x_vals = np.linspace(x_min, x_max, n_pts)
            
            f_slice = sp.lambdify(sym_x, slice_expr, modules=LAMBDIFY_MODULES)
            z_vals = np.asarray(f_slice(x_vals), dtype=float)
            z_clean = np.where(np.isfinite(z_vals) & (np.abs(z_vals) < 1e6), z_vals, np.nan)
            
            result["traces"].append({
                "type": "scatter",
                "mode": "lines",
                "name": f"z({var_x}, 0) [1D Slice at {var_y}=0]",
                "x": x_vals.tolist(),
                "y": [None if np.isnan(v) else float(v) for v in z_clean],
                "line": {"color": "#06b6d4", "width": 3},
            })
            result["stats"] = {
                "slice_variable": f"{var_y} = 0",
                "domain": [x_min, x_max],
                "num_points": n_pts,
            }
            result["layout_recommendations"] = {
                "xaxis": {"title": f"{var_x} Axis (1D)", "gridcolor": "rgba(255,255,255,0.1)"},
                "yaxis": {"title": f"z({var_x}, 0)", "gridcolor": "rgba(255,255,255,0.1)"},
            }
            return result

        elif eq_type == "POLAR":
            # 1D Curve of r vs theta
            sym_expr = parsed_meta["sympy_expr"]
            ...
            result["traces"].append({
                "type": "scatter",
                "mode": "lines",
                "name": "r(θ) Function Line",
                "x": theta_vals.tolist(),
                "y": [None if np.isnan(v) else float(v) for v in r_vals],
                "line": {"color": "#a855f7", "width": 3},
            })
            result["stats"] = {"num_points": n_pts, "domain": [t_min, t_max]}
            result["layout_recommendations"] = {
                "xaxis": {"title": "θ (Angle in Radians)", "gridcolor": "rgba(255,255,255,0.1)"},
                "yaxis": {"title": "r(θ) (Radius)", "gridcolor": "rgba(255,255,255,0.1)"},
            }
            return result

        else:
            # 1D explicit function y = f(x)
            sym_expr = parsed_meta.get("sympy_expr") or sp.Symbol('x')
            indep_vars = parsed_meta.get("independent_vars", ['x'])
            var_x = indep_vars[0] if len(indep_vars) > 0 else 'x'
            sym_x = sp.Symbol(var_x)
            
            x_min, x_max = ranges.get(var_x, (-10.0, 10.0))
            n_pts = max(300, resolution * 2)
            x_vals = np.linspace(x_min, x_max, n_pts)
            
            f_lamb = sp.lambdify(sym_x, sym_expr, modules=LAMBDIFY_MODULES)
            y_vals = np.asarray(f_lamb(x_vals), dtype=float)
            if isinstance(y_vals, (int, float)):
                y_vals = np.full_like(x_vals, y_vals)
            y_clean = np.where(np.isfinite(y_vals) & (np.abs(y_vals) < 1e6), y_vals, np.nan)
            
            result["traces"].append({
                "type": "scatter",
                "mode": "lines",
                "name": f"f({var_x}) 1D Function",
                "x": x_vals.tolist(),
                "y": [None if np.isnan(v) else float(v) for v in y_clean],
                "line": {"color": "#38bdf8", "width": 3},
            })
            result["stats"] = {"domain": [x_min, x_max], "num_points": n_pts}
            result["layout_recommendations"] = {
                "xaxis": {"title": f"{var_x} Axis (1D)", "gridcolor": "rgba(255,255,255,0.1)"},
                "yaxis": {"title": f"f({var_x})", "gridcolor": "rgba(255,255,255,0.1)"},
            }
            return result

    # --- DIMENSION 2D: Force 2D Cartesian / Contour / Polar ---
    if dimension == "2D":
        if "EXPLICIT_3D" in eq_type:
            # 2D Heatmap / Contour Projection for 3D function
            sym_expr = parsed_meta["sympy_expr"]
            indep_vars = parsed_meta.get("independent_vars", ['x', 'y'])
            var_x = indep_vars[0] if len(indep_vars) > 0 else 'x'
            var_y = indep_vars[1] if len(indep_vars) > 1 else 'y'
            ...
            result["traces"].append({
                "type": "scatter",
                "mode": "lines",
                "name": "r(θ) Polar Curve in Cartesian 2D",
                "x": x_vals.tolist(),
                "y": y_vals.tolist(),
                "line": {"color": "#a855f7", "width": 3},
            })
            ...
            y_vals = np.asarray(f_lambdified(x_vals), dtype=float)
            if isinstance(y_vals, (int, float)):
                y_vals = np.full_like(x_vals, y_vals)
    ...
    # --- DIMENSION 3D: Force 3D Surface / Mesh / Parametric Curve ---
    if dimension == "3D":
        if "EXPLICIT_2D" in eq_type:
            ...
            Z = np.tile(f_lamb(x_vals), (n_grid, 1))
            ...
            result["traces"].append({
                ...
                "colorscale": "Viridis",
                "showscale": True,
                "name": f"3D Extrusion of {parsed_meta.get('dependent_var', 'y')}({indep_var})",
            })
            result["stats"] = {"grid_size": f"{n_grid}x{n_grid}", "x_domain": [x_min, x_max], "y_domain": [y_min, y_max]}
            return result
            ...
            if 'z' not in free_vars:
                # 2D implicit extruded to 3D cylinder
                sym_x, sym_y = sp.Symbol('x'), sp.Symbol('y')
