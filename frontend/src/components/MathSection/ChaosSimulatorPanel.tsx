...
export const ChaosSimulatorPanel: React.FC<ChaosSimulatorPanelProps> = ({
  onGraphData,
}) => {
  ...
  const systems = [
    { id: 'lorenz', title: 'Lorenz Butterfly Attractor', desc: 'The iconic Butterfly Effect system (Edward Lorenz, 1963)' },
    { id: 'rossler', title: 'Rössler Continuous Chaos', desc: 'Simple 3D spiral chaos with folded band topology' },
    { id: 'aizawa', title: 'Aizawa Sphere-Torus Chaos', desc: 'Hypnotic spherical chaotic attractor with central core' },
    { id: 'lotka', title: 'Lotka-Volterra Predator-Prey', desc: 'Non-linear ecological population oscillation dynamics' },
  ];

  const fetchSimulation = async (sysName: string = system, s = sigma, r = rho, b = beta, pts = numPoints) => {
    setLoading(true);
    try {
      const res = await evaluateChaosSimulator(
        sysName,
        { sigma: s, rho: r, beta: b, a: 0.2, c: 5.7 },
        pts,
        0.01
      );
