            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-purple-900/40 pb-3.5">
                <div>
                    ...
                    <p className="text-[11px] text-purple-300/80">
                        Compare two documents side-by-side to analyze semantic divergence, sentiment shifts, and topic
                        overlap
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                    <div className="p-3 bg-slate-900/80 rounded-xl border border-sky-900/50">
                        ...
                        <div className="text-[11px] text-slate-300 space-y-0.5">
                            <div>
                                Sentiment: <b className="text-emerald-400">{compareResult.doc1.sentiment.label}</b> (
                                {compareResult.doc1.sentiment.compound_score})
                            </div>
                            <div>
                                Top Topic:{" "}
                                <span className="text-sky-300">{Object.keys(compareResult.doc1.topics)[0]}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="p-3 bg-slate-900/80 rounded-xl border border-rose-900/50">
                        ...
                        <div className="text-[11px] text-slate-300 space-y-0.5">
                            <div>
                                Sentiment: <b className="text-rose-400">{compareResult.doc2.sentiment.label}</b> (
                                {compareResult.doc2.sentiment.compound_score})
                            </div>
                            <div>
                                Top Topic:{" "}
                                <span className="text-rose-300">{Object.keys(compareResult.doc2.topics)[0]}</span>
                            </div>
                        </div>
                    </div>
                </div>
